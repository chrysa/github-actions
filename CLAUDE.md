...
Reusable GitHub Actions for all chrysa repositories. Provides gitversion, python-setup,
ruff-check, sonar-scan, sonar-scan-node, run-tests, tool-setup, install-project actions.
See README.md for the detailed repository structure.

## Development & testing

```bash
make install   # install actionlint + pre-commit
make test      # alias → validate (action schema validation)
```
...
- Version history and notable decisions: see `CHANGELOG.md`.
